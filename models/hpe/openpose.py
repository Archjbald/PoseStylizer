...
class OpenPose(nn.Module):
    ...
    def init_weights(self, dict_path):
        model_weights = torch.load(dict_path)

        transfer_model_weights = OrderedDict()
        for weights_name in self.state_dict().keys():
            transfer_model_weights[weights_name] = model_weights['.'.join(weights_name.split('.')[1:])]

        self.load_state_dict(transfer_model_weights)

    def get_feature_extractor(self, perceptual_layers, gpu_ids):
        submodel = nn.Sequential()
        for i, layer in enumerate(list(self.model0)):
            submodel.add_module(str(i), layer)
            if i == perceptual_layers:
                break
        submodel = torch.nn.DataParallel(submodel, device_ids=gpu_ids).cuda()
        return submodel

    def get_final_heatmaps(self, x, out6_2):
        heatmaps = TRANS.functional.resize(out6_2, x.shape[2:],
                                           interpolation=TRANS.InterpolationMode.BICUBIC)
        heatmaps = heatmaps[:, APS_IN_OPOSE]
        return heatmaps
