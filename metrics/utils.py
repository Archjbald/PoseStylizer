...
class ImageDatasetSplit(Dataset):
    ...
    def __init__(self, root, img_idx=0, exts=['png', 'jpg', 'JPEG'], transform=None,
                 num_images=None):
        """Construct an image dataset.
...
def get_fid(
        ...
        use_torch: bool = False,
        gt_loader: Union[torch.FloatTensor, DataLoader] = None,
        **kwargs,
) -> float:
...
def calc_and_save_stats(
    ...
    mu, sigma = calc_stats(loader, use_torch=use_torch, verbose=verbose)
...
def calc_stats(loader, use_torch=False, verbose=True):
    acts, = get_inception_feature(
