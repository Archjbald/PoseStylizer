...
def activations_from_loader(img_loader, model, dims=2048, batch_size=1, cuda=True):
    model.eval()

    pred_arr = np.empty((len(img_loader), dims))
    ...
    for i, img in enumerate(tqdm(img_loader)):
        ...
        pred_arr[i * batch_size:i * batch_size + 1] = pred.cpu().data.numpy().reshape(batch_size, -1)

    return pred_arr


def calculate_kid_given_loader(img_loader, gt_loader):
    """Calculates the KID of two paths"""
    dims = 2048
    batch_size = 1
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    ...
    act_true = activations_from_loader(gt_loader, model, dims, batch_size, cuda)

    acts = activations_from_loader(img_loader, model, dims, batch_size, cuda)
    kid_values = polynomial_mmd_averages(act_true, acts, n_subsets=100)
